...
public class iOSTraceReader extends Thread {
	
	public static final int DATE_AND_TIME_LENGTH = 19;
	
	String fileName; 
	
	public iOSTraceReader(String fileName) {
	...
	public void run () {
		...
		try {
			writer = new PrintWriter("iOSTrace.txt");
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		while (scanner.hasNextLine()) {
			...
			if (line.contains("[TRACE]") || line.contains("[DEBUG]")
					|| line.contains("[WARNING]") || line.contains("[INFO]")
					|| line.contains("[ERROR]")) {

				writer.write(line + "\n");
				if ((line.contains(": {") && (line.contains("[TRACE]")))) {
					line = scanner.nextLine();
